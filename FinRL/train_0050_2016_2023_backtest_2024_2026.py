...
def main() -> None:
    ...
    parser.add_argument("--include-dividends", action="store_true", help="Add dividends as cash during training/backtest.")
    ...
    print(f"Dividends:{' yes' if args.include_dividends else ' no'}")
    print("=" * 72)
    ...
    dividend_suffix = "_dividend" if args.include_dividends else ""
    model_path = (
        PROJECT_ROOT
        / "models"
        / "portfolio"
        / f"{ticker_slug}_{_date_slug(train_start)}_{_date_slug(train_end)}_{args.agent}_enhanced{dividend_suffix}"
    )
    ...
    output_file = (
        ...
        / (
            ...
            f"{args.agent}{dividend_suffix}_backtest_{_date_slug(backtest_start)}_{_date_slug(backtest_end)}_"
