#!/usr/bin/env python3
"""Grid-search the all-holdings PPO portfolio strategy."""
...
TRAIN_SCRIPT = PROJECT_ROOT / "train_all_holdings_portfolio_2020_2023_backtest_2024_2026.py"
...
class SearchConfig:
    ...
    benchmark_shortfall_penalty_weight: float
...
def _base_train_args(args: argparse.Namespace) -> list[str]:
    cmd = [
        ...
        str(args.equal_benchmark_weight),
        "--actual-benchmark-weight",
        str(args.actual_benchmark_weight),
        "--underperform-0050-weight",
        str(args.underperform_0050_weight),
        "--drawdown-penalty-weight",
        str(args.drawdown_penalty_weight),
        "--benchmark-shortfall-penalty-cap",
        str(args.benchmark_shortfall_penalty_cap),
        "--benchmark-shortfall-stress-scale",
        str(args.benchmark_shortfall_stress_scale),
        "--stress-budget-caution-invested-cap",
        str(args.stress_budget_caution_invested_cap),
        "--stress-budget-caution-0050-cap",
        str(args.stress_budget_caution_0050_cap),
        "--stress-budget-risk-off-invested-cap",
        str(args.stress_budget_risk_off_invested_cap),
        "--stress-budget-risk-off-0050-cap",
        str(args.stress_budget_risk_off_0050_cap),
        "--stress-budget-deep-risk-off-invested-cap",
        str(args.stress_budget_deep_risk_off_invested_cap),
        "--stress-budget-deep-risk-off-0050-cap",
        str(args.stress_budget_deep_risk_off_0050_cap),
        "--stress-rebalance-cooldown-days",
        str(args.stress_rebalance_cooldown_days),
        "--stress-confirm-days",
        str(args.stress_confirm_days),
        "--ppo-verbose",
        str(args.ppo_verbose),
    ]
    if args.use_rsi_features:
        cmd.append("--use-rsi-features")
    if args.disable_market_regime_features:
        cmd.append("--disable-market-regime-features")
    if args.daily_open_only:
        cmd.append("--daily-open-only")
    if args.monday_open_only:
        cmd.append("--monday-open-only")
    if args.enable_dca:
        cmd.extend(
            [
                "--enable-dca",
...
def _config_args(config: SearchConfig) -> list[str]:
    cmd = [
        ...
        str(config.benchmark_shortfall_penalty_weight),
    ]
    ...
    if config.enable_range_harvest:
        cmd.extend(
            [
                "--enable-range-harvest",
                "--range-drift-threshold",
                str(config.range_drift_threshold),
            ]
        )
    return cmd
...
def _run_one(args: argparse.Namespace, config: SearchConfig, seed: int) -> dict[str, Any]:
    ...
    score_metric = "return_on_invested" if args.enable_dca else "total_return"
    score_return = (
        float(investment.get("simple_return_on_total_invested", 0.0))
        if args.enable_dca
    ...
    row = {
        ...
        "market_stress_count": int(payload.get("market_stress_count", 0)),
        "stress_budget_count": int(payload.get("stress_budget_count", 0)),
        "final_weight_0050": float(payload["final_weights"].get("0050.TW", 0.0)),
        **asdict(config),
    }
    if args.enable_dca:
        row.update(
...
def _build_configs(args: argparse.Namespace) -> list[SearchConfig]:
    ...
    shortfalls = _parse_float_list(args.benchmark_shortfall_penalty_weights)
    ...
    if args.include_baseline:
        for turnover, rebalance, shortfall in itertools.product(turnovers, rebalance_days, shortfalls):
            configs.append(
                SearchConfig(
                    turnover_penalty=turnover,
                    min_rebalance_days=rebalance,
                    benchmark_shortfall_penalty_weight=shortfall,
    ...
    for turnover, rebalance, shortfall, pva_weight, pva_drift in itertools.product(
        turnovers,
        rebalance_days,
        shortfalls,
        pva_weights,
    ...
    if args.include_range_harvest:
        for turnover, rebalance, shortfall, range_drift in itertools.product(
            turnovers,
            rebalance_days,
            shortfalls,
            range_drifts,
...
def _summarize(rows: list[dict[str, Any]], objective: str) -> list[dict[str, Any]]:
    ...
    for config_id in sorted({row["config_id"] for row in rows}):
        ...
        summary = {
            ...
            **{key: group[0][key] for key in asdict(SearchConfig(0, 0, 0, False, 0, 0)).keys()},
        }
...
def _write_outputs(
    ...
    out_json = PROJECT_ROOT / "results" / f"optimize_all_holdings_portfolio_grid_{timestamp}.json"
    out_csv = PROJECT_ROOT / "results" / f"optimize_all_holdings_portfolio_grid_{timestamp}.csv"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Grid-search the all-holdings PPO portfolio strategy.")
    parser.add_argument("--train-start", default="2020-01-01")
    parser.add_argument("--train-end", default="2024-12-31")
    parser.add_argument("--backtest-start", default="2025-01-01")
    parser.add_argument("--backtest-end", default="2026-05-14")
    parser.add_argument("--download-end", default="2026-05-15")
    parser.add_argument("--timesteps", type=int, default=20_000)
    parser.add_argument("--seeds", default="42")
    parser.add_argument("--turnover-penalties", default="0.04,0.08")
    parser.add_argument("--min-rebalance-days", default="20,40")
    parser.add_argument("--benchmark-shortfall-penalty-weights", default="0.00,0.03")
    parser.add_argument("--pva-weights", default="0.30")
    parser.add_argument("--pva-drift-thresholds", default="0.05,0.08")
    ...
    parser.add_argument("--dca-total-monthly", type=float, default=25_000.0)
    ...
    print("=" * 72)
    print("All-holdings portfolio grid optimization done")
