...
if rl_vals_clean and bh_vals_clean:
    ...
    rl_sharpe = np.nan if len(rl_vals_clean) < 2 else (rl_vals_clean[-1] - rl_vals_clean[0]) / (np.std(rl_vals_clean, ddof=1) + 1e-12) * np.sqrt(252)
    bh_sharpe = np.nan if len(bh_vals_clean) < 2 else (bh_vals_clean[-1] - bh_vals_clean[0]) / (np.std(bh_vals_clean, ddof=1) + 1e-12) * np.sqrt(252)
