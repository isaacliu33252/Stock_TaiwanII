#!/usr/bin/env python3
"""Run a2118's OWN, independently-owned golden PPO signal generation.

Forked from run_group_a_combined_signal.py on 2026-09-09 to decouple a2118's
day-to-day live signal generation from golden1_0531's own pipeline output
file. See project_a2118_own_golden_signal_pipeline_split_20260909 memory for
the pipeline-decoupling rationale (this part is architectural only).

2026-09-09, same day, later: DEFAULT_RESULT_JSON repointed from the
golden1_0531-weights copy to the independently-trained cap20 PPO model
(a2118_independent_ppo_v1_80k_cap20). THIS IS A REAL STRATEGY CHANGE, not
architectural -- a2118 now sources its golden1-regime-day target weights from
a model that this project's own significance testing
(scripts/misc/significance_check_cap20_vs_production_20260909.py,
results/significance_check_cap20_vs_production_20260909.json) found was NOT
statistically distinguishable from production on the one genuinely fair
(both-sides-OOS) test window (best candidate p=0.343 uncorrected, needing an
estimated ~7-14 more years of comparable data to reach significance). The
point-estimate case for this candidate (final value +13%, Sharpe roughly tied,
MDD ~3pp worse) is real but not proven beyond what a single historical path
could produce by chance. Explicitly authorized by the user after this
evidence was presented directly, against this project's own
feedback_strategy_promotion_caution precedent. See
project_a2118_cap20_production_switch_20260909 memory for the full decision
record.

golden1_0531 itself, its own live pipeline, and run_group_a_combined_signal.py
are untouched by this change -- this script and its output files
(results/a2118_own_golden_live_latest.*) are the only thing that changed.
"""
...
RELEASE_NAME = "a2118_cap20_20260909"
RELEASE_HANDOFF = PROJECT_ROOT / "GROUP_A_PLUS_20260909_A2118_CAP20_PRODUCTION_SWITCH_HANDOFF.md"
DEFAULT_TDCC_CONFIG = PROJECT_ROOT / "group_a_tdcc_improved_config_destination_primary.json"
# 2026-09-09: repointed to the independently-trained cap20 PPO model. See the
# module docstring above and project_a2118_cap20_production_switch_20260909
# memory -- this is a real strategy change, not the earlier pipeline-only fork.
DEFAULT_RESULT_JSON = PROJECT_ROOT / "results" / "a2118_own_golden_source_20260909_cap20.json"
DEFAULT_LATEST_JSON = PROJECT_ROOT / "results" / "a2118_own_golden_live_latest.json"
DEFAULT_LATEST_CSV = PROJECT_ROOT / "results" / "a2118_own_golden_live_latest.csv"
DEFAULT_MANIFEST = PROJECT_ROOT / "results" / "a2118_own_golden_bundle_latest.json"
