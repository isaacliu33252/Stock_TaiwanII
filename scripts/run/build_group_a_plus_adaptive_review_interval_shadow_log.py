#!/usr/bin/env python3
"""Append today's adaptive review interval classification to a live shadow log.

Research-only, pure logging step -- see
group_a_plus/integrations/adaptive_review_interval_shadow_log.py for why
this exists: the historical 7-window backtest had 3 windows with zero
suppressed days (trivial ties), so real evidence about how often each
review interval (1d/3d/5d) actually gets used is thinner than 7 windows
suggests. This starts accumulating real daily classifications instead.
Never changes target weights, execution guards, or the latest live signal.
...
from group_a_plus.integrations.adaptive_review_interval_shadow_log import (  # noqa: E402
...
from scripts.evaluate.evaluate_adaptive_review_interval_shadow import classify_review_interval  # noqa: E402

DEFAULT_LOG_PATH = PROJECT_ROOT / "results" / "group_a_plus_adaptive_review_interval_shadow_log.jsonl"
DEFAULT_LATEST_PATH = PROJECT_ROOT / "report" / "group_a_plus" / "latest" / "adaptive_review_interval_shadow.json"
...
def build_row(*, db_path: Path, panel: str | None, regime_lookback_days: int, end: str) -> dict:
    ...
    if frame.empty:
        return build_shadow_log_row(frame=frame, review_interval_days=0, review_interval_label="")
    interval, label = classify_review_interval(frame.iloc[-1])
    return build_shadow_log_row(frame=frame, review_interval_days=interval, review_interval_label=label)
...
def main() -> None:
    ...
    print(f"status={row.get('status')} date={row.get('date')} review_interval_days={row.get('review_interval_days')} appended={appended}")
