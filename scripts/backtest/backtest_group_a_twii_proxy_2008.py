...
def main() -> None:
    ...
    if payload_uses_group_a_institutional_features(payload):
        stock_data = attach_institutional_features_db_first(stock_data, tickers, args.start, args.end)
    if payload_uses_group_a_margin_features(payload):
        stock_data = attach_margin_features_db_first(stock_data, tickers, args.start, args.end)
    if payload_uses_group_a_margin_shared_features(payload):
        stock_data = attach_group_a_margin_shared_features_db_first(stock_data, tickers, args.start, args.end)
    if payload_uses_group_a_market_margin_shared_features(payload):
        stock_data = attach_group_a_market_margin_shared_features_db_first(stock_data, tickers, args.start, args.end)
    ...
    payload_out = {
        ...
        "feature_gap_note": (
            "Historical institutional and margin-trading inputs may be unavailable for 2008; when missing, "
            "those per-ticker, basket-shared, and market-shared margin features are zero-filled to preserve canonical observation shape."
        ),
        "env_kwargs": env_kwargs,
        "limitations": [
