#!/usr/bin/env python3
"""Does adding 00662.TW (Fubon NASDAQ-100) to the a2118 defensive basket
...
Same-session follow-up to evaluate_group_a_plus_defensive_basket_sp500_00646
_20260821.py (broad S&P500 exposure in the defensive sleeve): this asks
whether the more concentrated, tech-heavy NASDAQ-100 exposure (00662.TW,
Fubon NASDAQ-100 -- used instead of the raw US-listed QQQ ticker to keep
currency/cost-structure consistent with every other basket tested today,
all TWD-denominated Taiwan-listed products) behaves differently from broad
S&P500 in the same defensive-sleeve role, particularly on the COVID and 2022
episodes where 00646 and 00635U (gold) showed opposite effects.
...
QQQ_TICKER = "00662.TW"
QQQ_BASKETS = {
    "qqq0_cash60": {"0050.TW": 0.40, QQQ_TICKER: 0.00, "cash": 0.60},
    "qqq10_cash50": {"0050.TW": 0.40, QQQ_TICKER: 0.10, "cash": 0.50},
    "qqq20_cash40": {"0050.TW": 0.40, QQQ_TICKER: 0.20, "cash": 0.40},
}
DEFAULT_OUTPUT = PROJECT_ROOT / "results" / "group_a_plus_defensive_basket_qqq_00662_20260821.json"
...
def main() -> None:
    ...
    extended_tickers = tuple(original_tickers) + (QQQ_TICKER,)
    ...
    basket_mod.DEFENSIVE_BASKETS.update(QQQ_BASKETS)
    ...
    keep_baskets = ["current_a207", "bond0_cash60", "qqq0_cash60", "qqq10_cash50", "qqq20_cash40"]
    baskets = {
    ...
    for ep in stress_episodes:
        ...
        for basket_name in ("bond0_cash60", "qqq0_cash60", "qqq10_cash50", "qqq20_cash40"):
    ...
    for row in per_episode_detail:
        print(
            ...
            f"qqq0={row['qqq0_cash60_total_return']:+.4f} "
            f"qqq10={row['qqq10_cash50_total_return']:+.4f} "
            f"qqq20={row['qqq20_cash40_total_return']:+.4f}"
        )
