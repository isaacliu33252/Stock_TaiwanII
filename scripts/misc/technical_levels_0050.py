    ...
    payload = {
        ...
        "trend": {
            **ma,
            ...
            "atr14": round(atr14, 2),
            "bollinger_upper20": round(float(df["close"].rolling(20).mean().iloc[-1] + 2 * df["close"].rolling(20).std().iloc[-1]), 2),
            "bollinger_lower20": round(float(df["close"].rolling(20).mean().iloc[-1] - 2 * df["close"].rolling(20).std().iloc[-1]), 2),
        },
        "ranges": ranges,
        "pivots": pivots,
        "swing_levels": swings,
