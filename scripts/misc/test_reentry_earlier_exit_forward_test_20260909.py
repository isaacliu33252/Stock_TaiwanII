#!/usr/bin/env python3
"""Direction #5 follow-up (2026-09-09): forward-test earlier re-entry rule
variants across full switch-rule history (not retrospective regret on known
recoveries). Uses a2118's ACTUAL production switch rule
(group_a_plus.runners.a2111._build_switch_rule()) and ACTUAL production
defensive basket (bond30_cash30) -- not the unrelated A207_RULE/simplified
basket another direction in this batch mistakenly used.

Tests whether loosening the 2020-fix fast-exit path (lower momentum
threshold, looser ma_gap guard, or shorter momentum lookback) nets out
positive once every new re-entry event it creates (including false/dead-cat
ones) is priced in across the full history -- not just the 11 known-good
recoveries the original retrospective regret analysis looked at.
...
DEFENSIVE = dict(DEFENSIVE_BASKETS["bond30_cash30"])  # {0050:40%, 00679B.TWO:30%, cash:30%}
...
def main():
    ...
    out_path = PROJECT_ROOT / "results" / "reentry_earlier_exit_forward_test_20260909.json"
