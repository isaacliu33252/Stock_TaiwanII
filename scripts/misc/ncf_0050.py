#!/usr/bin/env python3
"""
NCF_0050 — Next Close Forecast for 0050
價格預測 + 方向預測（雙模型）

用法:
    python3 scripts/misc/ncf_0050.py                      # 預測明日收盤+方向
    python3 scripts/misc/ncf_0050.py --output results/ncf_0050_20260624.json
"""
...
def load_external_df(main_df: pd.DataFrame, db_path: Path) -> pd.DataFrame:
    ...
    - Taiwan market (TWII/TSMC/0050): shift=0. Close simultaneously with 0050 at 1:30pm.
    ...
    try:
        ...
        inst = con.execute(
            ...
            "WHERE ticker='0050.TW' AND dt BETWEEN ? AND ? ORDER BY dt",
            [start_ext, end_ext],
        ).fetchdf()
        if not inst.empty:
        ...
        margin = con.execute(
            ...
            "FROM margin_data WHERE ticker='0050.TW' AND dt BETWEEN ? AND ? ORDER BY dt",
            [start_ext, end_ext],
        ).fetchdf()
        if not margin.empty:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "weights, since 2026-07-07 verification (results/ncf_0050_panel_drift_"
    ...
    print(f"[NCF_0050] Loading data from {db_path}")
    ...
    if not args.no_external_features:
        ...
        _stale = {
            "外資買賣超": _con.execute(
                "SELECT MAX(dt) FROM institutional_data WHERE ticker='0050.TW'"
            ).fetchone()[0],
            "融資融券": _con.execute(
                "SELECT MAX(dt) FROM margin_data WHERE ticker='0050.TW'"
            ).fetchone()[0],
            "台指期夜盤": _con.execute(
    ...
    for h in HORIZONS:
        ...
        if args.val_predictions_output:
            ...
            # ncf_0050 on 2017-2019 data, n_bull=605/n_bear=125). Use a
