#!/usr/bin/env python3
"""Sweep A21.20 trend-persistent threshold settings.

Research-only wrapper around evaluate_00631l_compounding_regime_no_add_shadow.
This keeps the current best staged add-speed policy fixed and checks whether
the TREND_PERSISTENT trigger itself can be improved.
"""
...
DEFAULT_OUTPUT = PROJECT_ROOT / "results" / "00631l_compounding_trend_threshold_grid_20260715.json"
DEFAULT_CSV = PROJECT_ROOT / "results" / "00631l_compounding_trend_threshold_grid_20260715.csv"
...
def _args_from(
    ...
    *,
    trend_score_min: int,
    ar1_trend_min: float,
    trend_persistence_min: float,
    reversal_speed_trend_max: float,
) -> SimpleNamespace:
    return SimpleNamespace(
        db=base.db,
        windows=base.windows,
        initial_value=base.initial_value,
        ar1_trend_min=ar1_trend_min,
        ...
        baseline_add_fraction=base.baseline_add_fraction,
        mean_reversion_add_fraction=base.mean_reversion_add_fraction,
        trend_persistent_add_fraction=base.trend_persistent_add_fraction,
...
def _row(report: dict[str, Any]) -> dict[str, Any]:
    ...
    for window in windows:
        for regime, count in window.get("regime_counts", {}).items():
            regime_counts[regime] = regime_counts.get(regime, 0) + int(count)
    return {
        "trend_score_min": thresholds["trend_score_min"],
        "ar1_trend_min": thresholds["ar1_trend_min"],
        "trend_persistence_min": thresholds["trend_persistence_min"],
        "reversal_speed_trend_max": thresholds["reversal_speed_trend_max"],
        ...
        "trend_persistent_days": regime_counts.get("TREND_PERSISTENT", 0),
        "mean_reverting_days": regime_counts.get("MEAN_REVERTING", 0),
...
def main() -> None:
    ...
    parser.add_argument("--trend-score-min-values", default="3,4,5")
    parser.add_argument("--ar1-trend-min-values", default="0.00,0.05,0.10")
    parser.add_argument("--trend-persistence-min-values", default="0.55,0.60,0.65")
    parser.add_argument("--reversal-speed-trend-max-values", default="0.40,0.45,0.50")
    ...
    for trend_score_min in _parse_ints(args.trend_score_min_values):
        for ar1_trend_min in _parse_floats(args.ar1_trend_min_values):
            for trend_persistence_min in _parse_floats(args.trend_persistence_min_values):
                for reversal_speed_trend_max in _parse_floats(args.reversal_speed_trend_max_values):
                    print(
                        "Evaluating "
                        f"score={trend_score_min} ar1={ar1_trend_min:.2f} "
                        f"persist={trend_persistence_min:.2f} revmax={reversal_speed_trend_max:.2f}"
                    )
                    report = build_report(
                        _args_from(
                            args,
    ...
    payload = {
        "schema_version": 1,
        "experiment": "00631l_compounding_trend_threshold_grid",
