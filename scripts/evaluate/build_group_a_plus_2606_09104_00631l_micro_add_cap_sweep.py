#!/usr/bin/env python3
"""Build a fixed 00631L micro-add cap sweep for 2606.09104 GroupA+ review.

This compares capped 00631L add candidates: 2.5%, 3%, 4%, and 5% against the
guarded 0050/cash target. It is a fixed candidate sweep, not an optimizer.
"""
...
DEFAULT_OUTPUT = PROJECT_ROOT / "report/group_a_plus/latest/2606_09104_00631l_micro_add_cap_sweep.json"
DEFAULT_HISTORY_DIR = PROJECT_ROOT / "report/group_a_plus/2606_09104_00631l_micro_add_cap_sweep/history"
DEFAULT_CAPS = (0.025, 0.03, 0.04, 0.05)
...
def _candidate_events(
    *,
    ...
    micro = _portfolio_return(returns, {"0050.TW": 0.3, "00631L.TW": cap, "cash": 0.7 - cap})
...
def _passes(summary: dict[str, Any], *, max_mdd_extra_loss: float, max_es_extra_loss: float, min_events: int) -> bool:
    return bool(
        summary.get("event_count", 0) >= min_events
        and (summary.get("mean_micro_minus_guarded_20d") or 0.0) > 0.0
        and (summary.get("hit_rate_micro_beats_guarded") or 0.0) >= 0.55
        and (summary.get("mean_mdd_extra_loss") or 0.0) >= max_mdd_extra_loss
        and (summary.get("mean_es_extra_loss") or 0.0) >= max_es_extra_loss
    )
...
def build_sweep(
    *,
    ...
    if len(returns) < lookback + horizon + 60:
        blockers.append("insufficient_history_for_micro_add_cap_sweep")
    ...
    if not blockers:
        ...
        for cap in caps:
            all_rows, high_rows = _candidate_events(returns=returns, states=states, cap=cap, horizon=horizon)
            ...
            reviews.append(
                {
                    "cap_00631l": _float(cap, 4),
                    "candidate_weights": {"0050.TW": 0.3, "00631L.TW": cap, "cash": 0.7 - cap},
    ...
    if not best_passing and not blockers:
        warnings.append("no_micro_add_cap_passes_high_extreme_gate")
    return {
        "schema_version": 1,
        "report_type": "group_a_plus_2606_09104_00631l_micro_add_cap_sweep",
        "generated_at": datetime.now().isoformat(timespec="seconds"),
        "policy": "fixed_candidate_sweep_shadow_only_no_live_weight_change",
        ...
        "source_paper": {
            ...
            "imported_concept": "constrained_micro_add_cap_sweep",
        ...
        "cap_reviews": reviews,
        "best_passing_high_extreme_cap": best_passing,
        "decision": {
            "any_cap_passes_high_extreme_gate": bool(best_passing),
            "best_cap_for_promotion_review": best_passing.get("cap_00631l") if best_passing else None,
            "allow_00631l_micro_add_from_this_sweep": False,
            "advance_to_promotion_gate": False,
            "target_weight_change_allowed": False,
            "replace_a2118": False,
            "summary": "Fixed cap sweep only; passing a shadow gate would still require separate promotion approval.",
        },
...
def write_sweep(report: dict[str, Any], output: Path = DEFAULT_OUTPUT, history_dir: Path = DEFAULT_HISTORY_DIR) -> None:
    ...
    (history_dir / f"2606_09104_00631l_micro_add_cap_sweep_{as_of.replace('-', '')}.json").write_text(
        text + "\n",
        encoding="utf-8",
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--caps", default="0.025,0.03,0.04,0.05")
...
def main() -> int:
    ...
    report = build_sweep(
        ...
        caps=tuple(float(item.strip()) for item in args.caps.split(",") if item.strip()),
