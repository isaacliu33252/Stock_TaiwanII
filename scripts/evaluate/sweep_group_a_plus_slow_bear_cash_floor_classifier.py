#!/usr/bin/env python3
"""Research-only slow-bear defensive cash-floor classifier sweep."""
...
CRASH_REGIME_PREFIX = "shadow_slow_bear_crash_floor"
SLOW_REGIME_PREFIX = "shadow_slow_bear_cash_floor"


@dataclass(frozen=True)
class SlowBearVariant:
    name: str
    ...
    drawdown_max: float
    ret20_max: float
    ma_gap_neg_window: int
    ma_gap_neg_ratio_min: float
    risk_lookback: int
    total_risk_min: int
...
def _defensive_age(regime: pd.Series) -> pd.Series:
    ages: list[int] = []
    age = 0
    for value in regime.astype(str):
        if value == "group_a_plus_defensive":
            age += 1
        else:
            age = 0
        ages.append(age)
...
def _ret20(frame: pd.DataFrame) -> pd.Series:
    if "return_0050_1d" in frame:
        ret = _numeric(frame, "return_0050_1d")
        return (1.0 + ret).rolling(20, min_periods=1).apply(lambda values: float(values.prod() - 1.0), raw=True)
    close = _numeric(frame, "0050_close")
    return close.pct_change(20).fillna(0.0)


def _slow_bear_mask(frame: pd.DataFrame, variant: SlowBearVariant) -> tuple[pd.Series, pd.Series]:
    ...
    # Codex 2026-08-14: crash handling is a fixed comparator here; the tested
    # variable is the slow-bear classifier, not live target generation.
    crash = (
        defensive
        & weak_momentum
        & (ma_gap <= -0.05)
        & ((drawdown <= -0.05) | (total_risk >= 4) | (tail_risk >= 1))
    )
    age = _defensive_age(regime)
    ret20 = _ret20(frame)
    ma_gap_negative_ratio = (
        (ma_gap < 0.0)
        .astype(float)
        .rolling(int(variant.ma_gap_neg_window), min_periods=1)
        .mean()
        .fillna(0.0)
    )
    risk_persist = total_risk.rolling(int(variant.risk_lookback), min_periods=1).max().fillna(0).astype(int)
    recovery_failure = (
        (age >= int(variant.def_age_min))
        & (ma_gap_negative_ratio >= float(variant.ma_gap_neg_ratio_min))
        & (ret20 <= float(variant.ret20_max))
    )
    slow = (
        defensive
        & weak_momentum
        & ~crash
        & (ma_gap <= float(variant.ma_gap_max))
        & (drawdown <= float(variant.drawdown_max))
        & (risk_persist >= int(variant.total_risk_min))
        & recovery_failure
    )
    return crash, slow


def _with_classifier_regime(frame: pd.DataFrame, variant: SlowBearVariant) -> tuple[pd.Series, dict[str, int]]:
    crash, slow = _slow_bear_mask(frame, variant)
...
def _variant_grid() -> list[SlowBearVariant]:
    variants: list[SlowBearVariant] = []
    for crash_floor in (0.50, 0.55, 0.60):
        for slow_floor in (0.40, 0.45, 0.50):
            for def_age_min in (0, 5, 10, 20, 60):
                for ma_gap_max in (0.0, -0.02):
                    for drawdown_max in (-0.05, -0.08):
                        for ret20_max in (0.02, 0.0, -0.02):
                            for ma_gap_neg_window in (10, 20):
                                for ma_gap_neg_ratio_min in (0.6, 0.8):
                                    for risk_lookback in (5, 10):
                                        for total_risk_min in (4, 6):
                                            name = (
                                                ...
                                                f"_r20{int((ret20_max + 0.10) * 100):02d}"
                                                f"_nw{ma_gap_neg_window}"
                                                f"_nr{int(ma_gap_neg_ratio_min * 100)}"
                                                f"_rl{risk_lookback}"
                                                f"_tr{total_risk_min}"
                                            )
                                            variants.append(
                                                SlowBearVariant(
                                                    name=name,
                                                    ...
                                                    def_age_min=def_age_min,
                                                    ma_gap_max=ma_gap_max,
                                                    drawdown_max=drawdown_max,
                                                    ret20_max=ret20_max,
                                                    ma_gap_neg_window=ma_gap_neg_window,
                                                    ma_gap_neg_ratio_min=ma_gap_neg_ratio_min,
...
def build_sweep_report(
    ...
    for variant in _variant_grid():
        ...
        regimes, activity = _with_classifier_regime(frame, variant)
    ...
    best = ranked[0]
    report = {
        "experiment": "group_a_plus_slow_bear_cash_floor_classifier_shadow",
        "generated_at": datetime.now().isoformat(timespec="seconds"),
        "implementation_note": (
            "Codex 2026-08-14: research-only slow-bear classifier shadow. "
            "The classifier uses defensive episode age, MA recovery failure, "
            "20-day return, drawdown, and risk persistence. It writes no live "
            "targets, execution plans, or strategy manifests."
        ),
        "window": {
...
def main() -> None:
    ...
    parser.add_argument("--output-prefix", default="results/group_a_plus_slow_bear_cash_floor_classifier_shadow")
