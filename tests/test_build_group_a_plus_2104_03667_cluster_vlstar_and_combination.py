...
from scripts.evaluate.build_group_a_plus_2104_03667_cluster_vlstar_and_combination import (
    build_review,
    write_review,
)
...
def test_and_combination_runs_and_reports_comparisons(tmp_path: Path) -> None:
    ...
    a = review["test_a_paper_momentum_validation"]
    for key in ("baseline_unfiltered", "cluster_only_filtered", "vlstar_only_filtered", "cluster_and_vlstar_filtered"):
        assert key in a

    b = review["test_b_group_a_plus_switch_check"]
    assert "and_dominates_existing_rule" in b
    assert "and_beats_both_single_detectors" in b
    # AND volatile share must never exceed either individual detector's share.
    assert b["and_volatile_share"] <= b["cluster_volatile_share"] + 1e-9
    assert b["and_volatile_share"] <= b["vlstar_volatile_share"] + 1e-9
