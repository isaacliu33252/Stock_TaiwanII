...
from scripts.evaluate.backtest_group_a_plus_llm_state_reward_stress_regime_gate_shadow import (
    build_review,
    write_review,
)
...
def _panel(path: Path, *, rows: int = 140) -> Path:
    ...
    for ticker in tickers:
        for idx, date in enumerate(dates):
            stress = idx >= 100
            if ticker == "0050.TW":
                reward, ret = -0.01, -0.02 if stress else 0.002
            elif ticker in {"0056.TW", "00713.TW", "00878.TW"}:
                reward, ret = -0.02, -0.03 if stress else 0.001
            elif ticker in {"00679B.TWO", "00751B.TWO"}:
                reward, ret = -0.08, 0.001 if stress else 0.0003
            else:
                reward, ret = -0.01, 0.0
            records.append(
                {
                    "date": date,
                    "ticker": ticker,
                    "close": 100.0 + idx if not stress else 130.0 - (idx - 100),
                    "return": ret,
...
def test_build_review_runs_stress_gate_without_live_effects(tmp_path: Path) -> None:
    ...
    assert review["report_type"] == "group_a_plus_llm_state_reward_interface_stress_regime_gate_shadow_backtest"
    ...
    assert review["summary"]["stress_gate"]["available_fold_count"] == 1
    assert review["fold_results"][0]["stress_gate_days"] >= 1
    assert review["decision"]["stress_regime_gate_ready_for_review"] is True
...
def test_build_review_blocks_missing_baseline(tmp_path: Path) -> None:
    ...
    assert review["decision"]["stress_regime_gate_passed_shadow_gate"] is False
    assert review["decision"]["promote_to_live"] is False


def test_write_review_writes_latest_and_history(tmp_path: Path) -> None:
    output = tmp_path / "latest" / "stress.json"
    ...
    history_file = history / "llm_state_reward_interface_stress_regime_gate_shadow_backtest_20260720.json"
