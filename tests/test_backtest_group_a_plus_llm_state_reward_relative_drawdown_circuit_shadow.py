...
from scripts.evaluate.backtest_group_a_plus_llm_state_reward_relative_drawdown_circuit_shadow import (
    build_review,
    write_review,
)


def _panel(path: Path, *, rows: int = 140) -> Path:
    ...
    for ticker in tickers:
        for idx, date in enumerate(dates):
            stress = idx >= 105
            if ticker in {"0056.TW", "00713.TW", "00878.TW"}:
                ...
                reward, ret = -0.01, -0.003 if stress else 0.001
            else:
                reward, ret = -0.01, 0.0
            records.append(
                {
        ...
        "fold_results": [
            {
                ...
                "test_end": dates[139].date().isoformat(),
            }
        ],
    }
...
def test_build_review_runs_relative_drawdown_circuit_without_live_effects(tmp_path: Path) -> None:
    panel_path = _panel(tmp_path / "panel.parquet")
    review = build_review(
        panel_path=panel_path,
        baseline_path=_baseline(tmp_path / "baseline.json", panel_path),
        relative_drawdown_trigger=0.001,
        relative_recovery_threshold=0.0001,
        min_circuit_days=3,
    ...
    assert review["report_type"] == "group_a_plus_llm_state_reward_interface_relative_drawdown_circuit_shadow_backtest"
    ...
    assert review["summary"]["relative_drawdown_circuit"]["available_fold_count"] == 1
    assert review["fold_results"][0]["circuit_days"] >= 1
    assert review["decision"]["relative_drawdown_circuit_ready_for_review"] is True
...
def test_build_review_blocks_missing_baseline(tmp_path: Path) -> None:
    ...
    assert review["decision"]["relative_drawdown_circuit_passed_shadow_gate"] is False
    assert review["decision"]["promote_to_live"] is False


def test_write_review_writes_latest_and_history(tmp_path: Path) -> None:
    output = tmp_path / "latest" / "circuit.json"
    ...
    history_file = history / "llm_state_reward_interface_relative_drawdown_circuit_shadow_backtest_20260720.json"
