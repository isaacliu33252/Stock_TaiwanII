...
from scripts.evaluate.sweep_group_a_plus_llm_state_reward_frozen_panel_baseline_params import (
    build_review,
    write_review,
)
...
def test_build_review_sweeps_no_model_params_without_live_effects(tmp_path: Path) -> None:
    ...
    assert review["report_type"] == "group_a_plus_llm_state_reward_interface_frozen_panel_baseline_param_sweep"
    assert review["status"] == "available_for_manual_offline_review"
    assert review["summary"]["evaluated_count"] == 4
    assert review["summary"]["recommended_count"] >= 1
    assert review["decision"]["recommended_baseline_variant_available"] is True
...
def test_build_review_blocks_missing_audit(tmp_path: Path) -> None:
    ...
    assert review["decision"]["recommended_baseline_variant_available"] is False
    assert review["decision"]["promote_to_live"] is False
...
def test_write_review_writes_latest_and_history(tmp_path: Path) -> None:
    ...
    history_file = history / "llm_state_reward_interface_frozen_panel_baseline_param_sweep_20260720.json"
