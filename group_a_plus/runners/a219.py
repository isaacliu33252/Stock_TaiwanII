"""Standardized runner for A21.9 candidate — Low-risk Fast Exit (Direction B).

A21.9 improvements over A21.7:
  - Inherits: entry_gap=0.003, ma_window=100, exit_gap=0.010 (all from A21.7)
  - NEW: Low-risk Fast Exit
      When total_risk_score <= 1, use exit_ma_gap=0.0 instead of 0.010.
      This allows faster return to golden1 once market risk has fully dissipated,
      without waiting for price to climb 1% above MA100.
  Motivation: 2021-09-29 defensive period — risk fell to 0 on 2021-10-26 but
  exit_ma_gap=1% held the system in defensive until 2021-11-08 (13 extra days).
"""
...
A219_ID = "a219_low_risk_fast_exit"


def _build_switch_rule() -> SwitchRule:
    """A21.9: A21.7 base + Low-risk Fast Exit.
    ...
    Adds: when total_risk_score <= 1, use exit_ma_gap=0.0 (any positive gap + momentum).
    """
    return SwitchRule(
        "risk_ma100_dd11_total6_eg3_xg10_lrfx0_thr1",
        100,    # ma_window
        0.003,  # enter_ma_gap  (same as A21.7)
        0.010,  # exit_ma_gap   (normal threshold)
        100,    # drawdown_window
        -0.11,  # enter_drawdown
        5, 5,
        0, None, 0, None, 6, 6,
...
def _run_a219(
    ...
    strategy_id: str = A219_ID,
    experiment: str = "group_a_plus_a219_low_risk_fast_exit",
...
def main() -> None:
    ...
    parser.add_argument("--output", default="results/group_a_plus_runner_a219.json")
    parser.add_argument("--frame-output", default="results/group_a_plus_runner_a219_frame.csv")
    args = parser.parse_args()
    std = OutputStandardizer("group_a_plus.runners.a219")
    try:
        report, frame = run_a219(
    ...
    print(f"A219 JSON:  {Path(args.output).resolve()}")
    print(f"A219 frame: {Path(args.frame_output).resolve()}")
