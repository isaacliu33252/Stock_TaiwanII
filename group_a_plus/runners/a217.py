"""Standardized runner for the research-only A21.7 candidate (New-A variant).

New-A improvements over A213 base:
  - entry_gap: 0.003 (vs 0.0175) — stricter entry reduces whipsaw
  - ma_window: 100 (vs 75) — smoother trend signal
  Result: Sharpe +0.21, MDD -3.3%, Annual +11.3%, rebal 8→2
"""
...
A217_ID = "a217_tight_entry_mw100"


def _build_switch_rule() -> SwitchRule:
    """New-A: tighter entry gap (0.003) + wider MA window (100)."""
    return SwitchRule(
        "risk_ma100_dd11_total3_eg3_xg10",
        100,   # ma_window
        0.003,  # entry_gap — very tight, only true trend changes trigger entry
        0.010,  # exit_gap
        100,   # ma_window
        -0.11,  # dd_threshold
...
def _run_recovery_strategy(
    ...
    strategy_id: str = A217_ID,
    experiment: str = "group_a_plus_a217_tight_entry",
    status: str = "candidate",
) -> tuple[dict, pd.DataFrame]:
    """Run recovery strategy with New-A switch parameters."""
...
def run_a217(
    ...
    return _run_recovery_strategy(
        ...
        basket_name="cash30",
        strategy_id=A217_ID,
        experiment="group_a_plus_a217_tight_entry",
        status="candidate",
    )
...
def main() -> None:
    ...
    parser.add_argument("--output", default="results/group_a_plus_runner_a217.json")
    parser.add_argument("--frame-output", default="results/group_a_plus_runner_a217_frame.csv")
    args = parser.parse_args()
    std = OutputStandardizer("group_a_plus.runners.a217")
    try:
        report, frame = run_a217(
            args.start,
            args.end,
            args.initial_value,
            Path(args.db),
