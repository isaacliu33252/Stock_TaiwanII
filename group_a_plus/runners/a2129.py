"""A21.29 runner — aggressive recovery 00631L boost with age guard.

This is a shadow candidate. It applies a 15% 00631L recovery boost only to the
first 20 trading days of each recovery episode.
"""
...
A2129_ID = "a2129_recovery_00631l_boost_age_guard_aggressive_shadow"
A2129_H20_MAX = 0.33
A2129_H5_REENTRY = 0.55
A2129_RECOVERY_00631L_BOOST_FRACTION = 0.15
A2129_RECOVERY_00631L_BOOST_MAX_AGE_DAYS = 20
...
def run_a2129(
    ...
    h20_max: float = A2129_H20_MAX,
    conf_min: float = NCF_LB_CONF_MIN,
    h5_reentry_min: float = A2129_H5_REENTRY,
    chip_data_fallback_max_stale_days: int | None = CHIP_DATA_FALLBACK_MAX_STALE_DAYS,
    recovery_00631l_boost_fraction: float = A2129_RECOVERY_00631L_BOOST_FRACTION,
    recovery_00631l_boost_max_age_days: int | None = A2129_RECOVERY_00631L_BOOST_MAX_AGE_DAYS,
    ...
    report["experiment"] = "group_a_plus_a2129_recovery_00631l_boost_age_guard_aggressive_shadow"
    report["strategy"] = A2129_ID
    report["status"] = "research_candidate"
    report["design_notes"]["a2129_recovery_00631l_boost_age_guard_aggressive"] = (
        "A21.29 uses a more aggressive 15% recovery 00631L boost, but still disables "
        f"the boost after {recovery_00631l_boost_max_age_days} trading days in the same recovery episode."
    )
    return report, frame
...
def main() -> None:
    ...
    parser.add_argument("--h20-max", type=float, default=A2129_H20_MAX)
    parser.add_argument("--conf-min", type=float, default=NCF_LB_CONF_MIN)
    parser.add_argument("--h5-reentry-min", type=float, default=A2129_H5_REENTRY)
    parser.add_argument("--chip-data-fallback-max-stale-days", type=int, default=CHIP_DATA_FALLBACK_MAX_STALE_DAYS)
    parser.add_argument("--recovery-00631l-boost-fraction", type=float, default=A2129_RECOVERY_00631L_BOOST_FRACTION)
    parser.add_argument("--recovery-00631l-boost-max-age-days", type=int, default=A2129_RECOVERY_00631L_BOOST_MAX_AGE_DAYS)
    parser.add_argument("--output", default="results/group_a_plus_runner_a2129.json")
    parser.add_argument("--frame-output", default="results/group_a_plus_runner_a2129_frame.csv")
    args = parser.parse_args()
    std = OutputStandardizer("group_a_plus.runners.a2129")
    try:
        report, frame = run_a2129(
            args.start,
            args.end,
            args.initial_value,
            Path(args.db),
    ...
    print(f"A2129 JSON:  {Path(args.output).resolve()}")
    print(f"A2129 frame: {Path(args.frame_output).resolve()}")
