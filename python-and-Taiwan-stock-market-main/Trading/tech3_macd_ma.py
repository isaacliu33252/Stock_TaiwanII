    ...
    def next(self) -> None:
        ...
        #反之有庫存則判斷是否有賣出機會
            ...
            if sell_condition1 and sell_condition2 and sell_condition3:
                #賣出log，改為sell profit
                self.log(f'SELL Profit, {self.dataclose[0]:.2f}')
                #平倉賣出
...
if __name__ == '__main__':
    ...
    for stock in stock_list:
        ...
        yf_data = yf_data.history(start='2018-01-01', end='2024-12-31')
        ...
        #將資料放入同一個list中
        con_data = [stock, set_cash, cerebro.broker.getvalue(), cerebro.broker.getvalue() - set_cash, 
                    a_return['rtot'], drawDown['max']['drawdown'], sharpe['sharperatio']]
        #append到一開始新增的list
        final_list.append(con_data)
        #plot出來
        figure = cerebro.plot(style='candlestick', barup='red', bardown='green', volume=False)
        #儲存相片
        figure[0][0].savefig(f'{Path(__file__).parent}/png/result_{stock}.png')
        #關閉相片
        plt.close()
    #產製檔案
