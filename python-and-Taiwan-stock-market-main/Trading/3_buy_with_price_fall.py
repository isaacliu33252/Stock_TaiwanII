...
try:
    #讀取股票清單
    data = pd.read_excel(SCRIPT_DIR / 'stock_list.xlsx')
    #獲取代號
    ...
    #使用 pd.concat 替代已廢棄的 DataFrame.append()
    ...
    for t in stock_store:
        ...
        merge_df['stock'] = [t] * len(merge_df)
        #使用 pd.concat 合併
        main_df = pd.concat([main_df, merge_df], ignore_index=True)
